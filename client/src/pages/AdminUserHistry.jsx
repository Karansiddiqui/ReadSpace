...
const AdminUserHistry = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const res = await fetch(`/api/transaction/userBooks?userId=${userId}`);
      ...
      {books.length > 0 ? (
        <div className="bg-white shadow-lg rounded-lg dark:bg-gray-800">
          ...
          {books.map((transaction, index) => (
            <div
              key={index}
              className="grid grid-cols-1 md:grid-cols-5 border-t dark:border-gray-600 p-4 items-center"
            >
              ...
              <div className="text-center text-gray-700 dark:text-gray-300">
                {transaction.returnDate[transaction.returnDate.length - 1] !==
                  null && transaction.status === "returned"
                  ? formatDate(
                      transaction.returnDate[transaction.returnDate.length - 1]
                    )
                  : formatDate(
